...
JnwjSqlTableModel::JnwjSqlTableModel(QObject *parent) :
    SSqlTableModel(parent)
{
    mAdvancedKey = new DbJnwj;
}

JnwjSqlTableModel::~JnwjSqlTableModel()
{

}

bool JnwjSqlTableModel::setAdvancedKey(DbRec *key)
{
    DbJnwj *oldKey = dynamic_cast<DbJnwj*>(mAdvancedKey);
    if (!oldKey)
        return false;

    DbJnwj *newKey = dynamic_cast<DbJnwj*>(key);
    if (!newKey)
        return false;
...
QString JnwjSqlTableModel::tableName() const
{
    return "archive_book";
}

void JnwjSqlTableModel::rowInfo(const int row, DbRec *info)
{

}

void JnwjSqlTableModel::print(QTextDocument *doc)
{

}

bool JnwjSqlTableModel::useOcr()
{
    DbJnwj *key = dynamic_cast<DbJnwj*>(mAdvancedKey);
    if (!key)
        return false;
...
bool JnwjSqlTableModel::selectByAdvancedKey()
{
    //check key
    DbJnwj *key = dynamic_cast<DbJnwj*>(mAdvancedKey);
    if (!key)
